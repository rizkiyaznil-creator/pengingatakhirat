    ...
    <div className="space-y-4">
      ...
      <div className="card space-y-4 px-5 py-5">
        ...
        <button
          onClick={relocate}
          disabled={busy}
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-pondok-300 bg-pondok-50 px-4 py-3 font-medium text-pondok-700 transition active:scale-[0.98] disabled:opacity-60"
        >
          <LocationIcon size={20} />
          {busy ? 'Mendeteksi…' : 'Perbarui lokasi otomatis'}
        </button>
        {msg && <p className="text-center text-xs text-pondok-600">{msg}</p>}
        <p className="text-center text-[11px] text-pondok-900/40">
              ...
              <button
                ...
                className={`rounded-2xl py-3 text-sm font-semibold transition ${
                  profile.madhab === m
                    ...
                    : 'bg-white text-pondok-900/60 ring-1 ring-cream-200'
                }`}
              >
      ...
      <div className="card px-5 py-4">
        ...
        <p className="mt-1 text-xs leading-relaxed text-pondok-900/55">
          Niyatin — habit tracker & pengingat ibadah. Data tersimpan di perangkatmu (privat,
          bisa dipakai offline). Sinkronisasi antar perangkat & adzan otomatis menyusul di
          pembaruan berikutnya, insyaAllah.
        </p>
        <p className="mt-2 text-[11px] text-pondok-900/35">Versi 0.1.0 · Fase 1</p>
      </div>
    </div>
  )
}
